...
/** Parse -> checksum -> schema-validate, in that order (TDD section 24.5). */
export function loadSaveEnvelope(value: unknown): LoadedSave {
  ...
  let state: GameState;
  try {
    state = validateGameState(envelope.state);
  } catch (error) {

...
/** Opt-in post-training fixture for cross-package integration tests. */
export function withBaselineModels(
  state: Readonly<GameState>,
  content: CompiledContent,
): GameState {
  return addBaselineModelsForTest(state, content);
}
...
export class ScenarioBuilder {
  ...
  /** Opt into a trained, productised test model for post-training subsystem tests. */
  withBaselineModel(): this {
    this.baselineModel = true;
    return this;
  }
  ...
  build(): GameState {
    ...
    const initial = this.baselineModel ? addBaselineModelForTest(base, content) : base;
    const draft = structuredClone(initial) as MutableState;

...
import { clonePlainData, type DeepMutable } from "./draft.ts";
...
export interface SimulationTransaction {
  ...
  /** Domain events emitted so far in this transaction, in emission order. */
...
export function createTransaction(before: GameState): SimulationTransaction {
  const draft = clonePlainData(before);

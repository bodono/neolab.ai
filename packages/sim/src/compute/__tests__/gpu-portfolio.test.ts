...
import { basisPoints, fraction, gpuCount } from "../../model/units.ts";
import {
...
describe("normaliseAllocation", () => {
  it("uses exact integer hierarchy sums and marks sub-200 programmes stranded", () => {
    const allocation: GpuAllocationState = {
      servingBasisPoints: basisPoints(0),
    ...
    const plan = normaliseAllocation(allocation, lots);
    ...
    expect(plan.safetyPrograms.every((program) => !program.isFunded)).toBe(true);
    expect(
      plan.safetyPrograms.reduce((sum, program) => sum + program.strandedPhysicalGpus, 0),
    ).toBe(100);
  ...
  it("preserves every hierarchy invariant over 5,000 deterministic random portfolios", () => {
    ...
    for (let iteration = 0; iteration < 5000; iteration += 1) {
      ...
      const allocation: GpuAllocationState = {
        servingBasisPoints: basisPoints(random() % 10_001),
    ...
    lab.compute.reservations = [
      {
        ...
        gpus: gpuCount(4500),
        generationIds: [contentId("base:gpu.pascal")],
        minimumInterconnectTier: 2,
      },
      {
...
describe("calculateGpuThroughput", () => {
  it("breaks out generation, availability, software, power, fabric, and modifiers", () => {
    const draft = mutable(createBareState());
    const lab = draft.labs[draft.run.playerLabId];
    if (lab === undefined) throw new Error("test player lab missing");
    const secondLot = lab.compute.lots[1];
    ...
    lab.compute.softwareEfficiency = 1.1;
    ...
    expect(breakdown.throughputBeforeSoftware).toBeCloseTo(4500);
    expect(breakdown.throughputBeforeModifiers).toBeCloseTo(4950);
    expect(breakdown.final).toBeCloseTo(5940);
...
describe("SetGpuAllocationCommand context switching", () => {
  it("records the Stage 3 penalty flag when a domain moves by more than 25 points", () => {
    ...
    const allocation: GpuAllocationState = {
      ...
      capabilityDomainWeights: {
        "base:domain.architectures": basisPoints(6600),
        "base:domain.optimisation-scaling": basisPoints(2500),
        "base:domain.data_representation": basisPoints(900),
      },
    };
    const command = {
  ...
  it("does not treat an exact 25-point move as a large swing", () => {
    ...
    const exactBoundary: GpuAllocationState = {
      ...previous,
      capabilityDomainWeights: {
        "base:domain.architectures": basisPoints(6500),
        "base:domain.optimisation-scaling": basisPoints(2500),
        "base:domain.data_representation": basisPoints(1000),
      },
    };

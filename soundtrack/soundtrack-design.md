...
**Target:** twenty-five full tracks and twenty event cues for a roughly two-hour run
...
During the endgame the Lab Motif becomes faster, compressed, and harmonically unstable --
double-time fragments that break off before their answer. The endgame is the race's final act
and is scored as a race: excited, anxious, thrilling, never sad or sombre, while the comfort
rules hold absolutely. A successful ending finally completes the motif on a warm added-sixth
chord. A loss may stop before the final answer rather than playing a melodramatic sting.
...
- Every ending has real music. Full victories earn the most upbeat ending in the score; qualified
  victories are celebrated one register lower; non-catastrophic losses close on a dignified elegy;
  catastrophic losses share the extinction family's emptiness without borrowing its finality.
...
| `last-evaluation` | The Last Evaluation | Reflective endgame alternate | Expansive, tense | 104 | 3:05 |
...
| `overnight-run` | The Overnight Run | Normal laboratory, night shift | Calm, glowing, industrious | 100 | 2:53 |
| `gpus-arrive-tuesday` | The GPUs Arrive on a Tuesday | Normal laboratory, delivery day | Upbeat, optimistic, fun | 132 | 2:26 |
| `tests-pass-first-try` | All Tests Pass on the First Try | Normal laboratory, clean pipeline | Upbeat, clean, relieved | 128 | 2:30 |
| `new-hire-orientation` | New Hire Orientation | Normal laboratory, first day | Upbeat, wide-eyed, warm | 124 | 2:35 |
| `demo-worked-twice` | The Demo Worked Twice | Normal laboratory, demo day | Upbeat, triumphant, playful | 134 | 2:23 |
| `budget-approved` | Compute Budget Approved | Normal laboratory, finance said yes | Upbeat, strutting, warm | 120 | 2:24 |
| `converged-before-lunch` | Converged Before Lunch | Normal laboratory, early convergence | Upbeat, bouncing, satisfied | 130 | 2:20 |
| `the-graph-goes-vertical` | The Graph Goes Vertical | Endgame: candidacy confirmed | Racing, awed | 140 | 2:45 |
| `hands-off-the-weights` | Hands Off The Weights | Endgame: containment work | Coiled, urgent | 132 | 2:58 |
| `every-phone-at-once` | Every Phone At Once | Endgame: outside pressure | Besieged, kinetic | 144 | 2:40 |
| `go-no-go` | Go / No-Go | Endgame: the decision | Anxious, ticking | 152 | 2:32 |
| `ship-it` | Ship It | Endgame: rollout | Committed, hurtling | 148 | 2:49 |
| `adrenaline-half-life` | Adrenaline Half-Life | Post-endgame return after an archived candidate | Relieved, wired | 126 | 2:32 |
| `ghost-in-the-cluster` | Ghost in the Cluster | Machine-flavoured crises: autonomy incidents, anomaly clusters | Uncanny, tense | 136 | 2:49 |
| `the-machine-moves-first` | The Machine Moves First | Endgame: containment failure | Vast, inexorable | 146 | 2:38 |
| `the-window-is-closing` | The Window Is Closing | Endgame: evidence sprint | Fast, tense, anxious | 146 | 2:41 |

The retired held-breath suite (17-22: The Number Appears, Containment Posture, Everyone Is
Calling, The Final Review, Root Access, False Dawn) remains on disk and in `audition.html` for
reference but is out of the game rotation; `endgame-music-plan.md` records its design.
| `a-qualified-success` | A Qualified Success | Qualified-victory endings | Celebratory, wry | 104 | 2:28 |
| `exit-interview` | Exit Interview | Non-catastrophic loss endings | Elegiac, dignified | 84 | 2:51 |
| `loss-of-signal` | Loss of Signal | Catastrophic non-extinction endings | Sparse, administrative | 66 | 2:55 |

The asterisk in `Cashflow Positive*` is part of the title and should appear in the soundtrack list.

### 4.1 Arrangement standard

Every track except Track 09 contains introductions, contrasting development, a sparse breathing
section, a thematic return, and a musical loop landing. Added duration comes from harmony, rhythm,
register, and orchestration changes rather than repeating a short render. Track 09 is a
deliberately non-looping ending with a natural fade.

Track 10 bends one convention on purpose: its arpeggio cycles in six-note, three-beat groups
across the 4/4 grid, drifting against the barline and realigning every three bars. In exchange it
is stricter than the palette elsewhere — no brush snares, hats, or tick blips at all, only the
rounded kick — so the night-shift track can never startle.

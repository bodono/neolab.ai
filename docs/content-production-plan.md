...
| Discoverable papers | 100+ | Current catalogue: 88 real, 23 fictional future papers | Authored; editorial review ongoing |
...
| Crisis chains | 30 | All mechanical, 2–5 beats each | Engine-side; see §12 note |
| Endgame decision nodes | 48 | All mechanical and state-reactive | Engine-side; stage machine built |
| Endgame crisis inserts | 12 | Modular, not present every run | Engine-side; see §12 note |
| Distinct endings / epilogues | 18 | Mechanical result plus reactive prose | Ending families exist |
| Lab-feed templates | 600 | At least 400 have no direct effect | Engine-side; ~170 written (see §12) |
...
> **The Deployment Crisis is engine-side.** Its decision points live in
> `packages/sim/src/endgame/` as a stage machine -- confirmation, containment
> posture, evidence sprint, pressure collision, final review -- with fifteen
> crisis project types, four evidence-sprint workstreams and three pressure
> responses, plus the endings and dialogue templates beside them. The
> `crisisChains`, `endgameDecisionNodes` and `endgameCrisisInserts` quotas
> counted events carrying `phase: "crisis"` or `category: "endgame"`. No event
> has ever set that phase, so two of the three could only report zero, and all
> three described a design the crisis was not built as. They have been retired
> from the manifest. The targets below remain the design intent for how much
> endgame there should be; the work happens in that module.
...
|---|---|---|---|---|
| Dennis Hassabi — The Visionary | DeepBrain | Aquarius | Research institution and scientific breakthroughs | Precise wonder; a research roadmap has become a heroic epic |
| Mario Amodeo — The Philosopher | Humanic | Maude | Safety evidence and principled scaling | Earnest, analytical, memo-shaped, unexpectedly dry |
| Stan Altmann — The Rainmaker | ClopenAI | GBT | Capital, products, and momentum | Polished optimism interrupted by impossible infrastructure requests |
| Elon Tusk — The Industrialist | xMind | Gronk | Physical scale, engineering, and robotics | First principles, enormous machinery, deadline visible from orbit |
| Liang Wenfang — The Optimizer | DeepSearch AI | DeepSearch | Algorithmic efficiency and technical depth | Understated competence; three zeroes quietly disappear from the budget |
...
Example: `Stan Altmann · CLOPENAI` followed by `AI · GBT-4 “ORBIT” · Level 4 Tool-Using Agent`.

Each AI family needs at least eight era-appropriate generated model names plus player renaming. A content record stores family, generation ordinal, public name, internal codename, and pronunciation/grammar tokens. Event text always resolves the current lab's family and model rather than hard-coding GBT.

## 5. AI capability ladder

The AI needs a legible sense of ascent without reducing intelligence to one XP bar. **Capability tier** is a public descriptive classification derived from Frontier Capability. It never represents alignment or safety; the separate AGI-candidate gate checks every capability trait.

Progress to the next tier is shown as an uncertain estimate such as `early`, `developing`, `approaching`, or a noisy range.
...
| 6 | General Problem Solver | 80–87 | Descriptive capability band | Near-AGI model; prosperity projects accelerate; rival emergency likely |
...
| 8 | Superintelligence | 95–100 | Candidate confirmed plus superhuman cross-domain evaluations | Endgame-only tier; capability exceeds ordinary institutional decision speed |
...
The catalogue target is now at least 100 landmark works. The current authored pack contains 111:
88 real works and 23 fictional future works. It spans foundations, deep learning, reinforcement
learning and robotics, transformers and generative models, scientific AI, safety and
interpretability, future capability methods, and prosperity breakthroughs.

Safety receives a deliberate historical arc rather than a token category: foundational
cybernetics and philosophical arguments introduce the control problem; real technical papers
develop alignment, interpretability, evaluation, and containment methods; clearly marked
fictional papers carry those traditions into the frontier era.
...
#### 6.1.1 Mathematical Grand Challenges

The late-game content pass should also prepare a companion set of mathematical and formal-science
Grand Challenges. These are milestones rather than ordinary papers and do not consume the
ordinary-paper target. Candidates include reliable proof-assistant formalisation, autonomous theorem
research, a recognised open conjecture, a Clay Millennium problem, the Riemann hypothesis, and
the resolution of P versus NP.

Every claimed solution needs separate claim, verification, replication, and publication text. P
versus NP must branch correctly rather than assume `P = NP`; practical cryptographic consequences
are separate from the theorem result. Every unresolved real-world problem carries the same
fictional-future labels as the fictional paper catalogue and must not have a fabricated real-world
citation. The mechanical and verification rules are specified in GDD §41.1.1 and
...
| Primary technical uncertainty | Capability breadth, deception, corrigibility, containment, robustness, misuse |
...
> **Built engine-side, not as content.** Ambient chatter lives in
> `packages/sim/src/feed/ambient.ts` as pools selected from live state -- the
> model's capability tier, and whether the lab is nervous, short of money, or
> losing ground to a rival. Roughly 170 lines are written and shipping. The
> `labFeedTemplates` quota below described a content-authored library with
> per-record weights, cooldowns and minimum tiers; nothing ever produced those
> records, so it reported zero against a design the implementation had not
> followed, and it has been retired from the manifest. The distribution and
> behaviour rules that follow are still the writing brief -- add lines to the
> pools in that module. Moving the library into content later remains open, and
> would need a record type and a consumer before any line is worth rewriting.
